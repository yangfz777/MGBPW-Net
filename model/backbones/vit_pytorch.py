...
class TransReID(nn.Module):
    ...
    def forward_features(self, x, camera_id, view_id):
        ...
        if self.local_feature:
            outputs = []  # 用于保存特定层的输出
            for i, blk in enumerate(self.blocks[:-1]):  # 获取层的索引
                x = blk(x)  # torch.Size([32, 243, 768])

                # 保存第三层的输出（索引从0开始，所以第三层的索引是2）
                # if i == 1:
                #     outputs.append(('second_layer_output', x))
                #
                # # 保存第六层的输出（索引从0开始，所以第六层的索引是5）
                # if i == 3:
                #     outputs.append(('fourth_layer_output', x))
                if i == 5:
                    outputs.append(('sixth_layer_output', x))
                if i == 7:
                    outputs.append(('eighth_layer_output', x))
                # if i == 9:
                #     outputs.append(('tenth_layer_output', x))
            return x, outputs
