...
class RandomErasing_foregroundpro(object):
    def __init__(self, EPSILON=0.5, root=None):
        self.EPSILON = EPSILON
        self.root = root
        # 只保留.jpg格式的图片
        self.occ_imgs = [img for img in os.listdir(self.root) if img.endswith('.png')]
        self.len = len(self.occ_imgs)
    ...
    def __call__(self, img):
        ...
        # 选择遮挡图片的上半部分
        crop_h = random.randint(occ_h // 2, occ_h)  # 裁剪高度
        crop_w = random.randint(occ_w // 3, occ_w)  # 裁剪宽度
        crop_top = 0  # 从上半部分开始裁剪
        crop_left = random.randint(0, occ_w - crop_w)  # 随机选取宽度部分
        ...
        # 随机确定遮挡的位置
        top = random.randint(int(h - h_), int(h - h_))
        left = random.randint(0, w - w_)

        # 将遮挡图片覆盖到原图上
        img[:, top:top + h_, left:left + w_] = occ_img

        # save_path = '/opt/data/private/yfz/PADE-main-single-3090/data/HSPMfile/processed_image.jpg'
        # to_pil = T.ToPILImage()
        # pil_img = to_pil(img)
        # # 保存图像
    ...
    # def save_occ_img(self, occ_img):
    #     # 将张量转换回PIL图像进行保存
    #     self.save_path = "/opt/data/private/yfz/PADE-main-single-3090/data/temp"
    #     occ_img = T.ToPILImage()(occ_img)
    #     if not os.path.exists(self.save_path):
    #         os.makedirs(self.save_path)
    #     occ_img_save_path = os.path.join(self.save_path, f"occ_img_{random.randint(0, 99999)}.jpg")
...
class Erasing(object):
    def __init__(self, EPSILON=0.5, root1=None,root2=None):
        self.EPSILON = EPSILON
        self.root1 = root1
        self.root2 = root2
        # 只保留.jpg格式和.png格式的图片
        self.occ_imgs_png = [img for img in os.listdir(self.root1) if img.endswith('.png')]
        self.occ_imgs_jpg = [img for img in os.listdir(self.root2) if img.endswith('.png')]
        self.len_png = len(self.occ_imgs_png)
        self.len_jpg = len(self.occ_imgs_jpg)

    def __call__(self, img):
        # 随机决定使用哪种遮挡方式
        if random.uniform(0, 1) > 0.5:
            # 使用RandomErasing_foregroundpro (0.1的概率)
            return self.foreground_pro(img)
        else:
            # 使用RandomErasing_Backgroundpro (0.9的概率)
            return self.background_pro(img)

    def foreground_pro(self, img):
        # 随机决定是否应用遮挡
        if random.uniform(0, 1) > self.EPSILON:
            return img

        # 随机选择一个遮挡图片
        index = random.randint(0, self.len_png - 1)
        occ_img = self.occ_imgs_png[index]
        occ_img = Image.open(os.path.join(self.root1, occ_img)).convert('RGB')
    ...
    def background_pro(self, img):
        # 随机决定是否应用遮挡
        if random.uniform(0, 1) > self.EPSILON:
            return img

        # 随机选择一个遮挡图片
        index = random.randint(0, self.len_jpg - 1)
        occ_img = self.occ_imgs_jpg[index]
        occ_img = Image.open(os.path.join(self.root2, occ_img)).convert('RGB')
        ...
        h_, w_ = occ_h, occ_w  # 获取遮挡图像的尺寸

        # 随机选择遮挡图像的粘贴位置（四个角）
        corners = [(0, 0), (0, w - w_), (h - h_, 0), (h - h_, w - w_)]  # 四个角的位置
        top, left = random.choice(corners)  # 随机选择一个角
        # # 随机确定遮挡的位置
        # top = random.randint(0, h - h_)
        # left = random.randint(0, w - w_)
        # 将遮挡图像覆盖到原图的随机角落
...
class RandomErasing_objectgroundpro(object):
    def __init__(self, EPSILON=0.5, root=None):
        self.EPSILON = EPSILON
        self.root = root
        # 只保留.png格式的图片
